...
uint8_t Sensor::getValue(uint8_t *sensor_type) {
    ...
    *sensor_type = this->type;
    ...
    if (this->type == ALGOSENSOR_TYPE_1WIRE) {
        // Serial.print("[Sensor] Trying 1WIRE on pin ");
        // Serial.println(pin);

        cmd_tx(0x30, NULL, 0);  // ili 0x20 ako koristiš RAW
        uint8_t cmd;
        uint8_t payload[32];
        uint8_t size;
        ...
        if (res == 0 && size >= 4) {
            ...
            value = raw_value;
            // Serial.print("[Sensor] 1WIRE value: ");
            // Serial.println(value);
        } else {
            // Serial.println("[Sensor] No 1WIRE response, trying PWM fallback...");
            ...
            while (cnt--) {
                if (digitalRead(pin) == 0) {
                    ...
                    *sensor_type = ALGOSENSOR_TYPE_PWM;
                    // Serial.println("[Sensor] PWM device detected");
                    break;
                }
    ...
    if (this->type == ALGOSENSOR_TYPE_PWM) {
        // Serial.print("[Sensor] Reading PWM on pin ");
        // Serial.println(pin);
        ...
        if (pwmHighValue > 0) {
            ...
            value = uint8_t(round(dutyCycle / 10.0));
            // Serial.print("[Sensor] PWM duty cycle: ");
            ...
            if (digitalRead(pin)) {
                ...
                *sensor_type = ALGOSENSOR_TYPE_1WIRE;
                // Serial.println("[Sensor] Back to 1WIRE");
            } else {
                value = 0;
                // Serial.println("[Sensor] PWM no signal, value = 0");
            }
        }
    }
